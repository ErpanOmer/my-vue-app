...
<template>
    <div :class="['er-relative er-flex er-flex-col er-space-y-2 er-cursor-pointer er-pr-4 er-px-8 er-py-4 hover:md:er-opacity-85 mb:er-px-6 mb:er-rounded-2xl', activeStore === item.id ? 'er-bg-[#eee]' : 'er-bg-white']">
        ...
        <div v-if="item.categories.length" class="er-flex er-flex-col">
            ...
            <ul class="er-opacity-80 text-size12 er-py-1 er-space-y-0.5 marker:er-text-transparent er-pl-2 mb:er-flex mb:er-space-x-4 mb:er-items-center">
                <li v-for="value in item.categories" :key="value">
                    <a-badge status="error" /><span class="er-ml-[-2px]">{{ constans.SERVICES[value].name || '' }}</span>
                </li>
            </ul>
        </div>
        ...
        <span class="er-flex-1"></span>
        <a-button v-if="!item.noBook" :size="constans.IS_MOBILE ? 'large': 'medium'" type="primary" danger @click.stop="Booknow(item, $event)"
            class="!er-absolute er-right-4 er-bottom-5 mb:!er-static mb:er-w-full">
