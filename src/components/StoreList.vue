...
<template>
    <div class="er-flex-1 er-pb-8 er-flex er-flex-col er-overflow-hidden" style="margin: 0 -20px;">
        <div class="er-flex er-items-center text-size14 er-px-10 er-py-4 er-pb-4 er-shadow-2xl">
            Find nearby stores: <span class="er-text-primary er-pl-2">{{formState.storeList.filter(s => s.show).length}}</span>
        </div>
        <div class="er-flex er-flex-col er-space-y-4 er-overflow-auto" ref="listContainer">
            <TransitionGroup name="list">
                <template v-for="(item, index) in sortedList" :key="item.id">
                    <div :class="['er-relative er-flex er-flex-col er-space-y-2 er-cursor-pointer er-pr-4 er-px-8 er-py-4 hover:er-opacity-85', item.distance === '0.00' ? 'er-bg-[#eee]' : 'er-bg-white']"
                        v-if="item.show" @click="onClick(item)">
                        <div class="er-flex er-items-center er-text-dark er-space-x-2">
                            ...
                            <span class="text-size14 er-text-primary er-font-bold">{{ item.distance }} Miles</span>
                        </div>
