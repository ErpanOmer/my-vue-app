...
<template>
    <div>
        <button id="start-quiz-btn">Find My Perfect E-Bike</button>
    ...
    <template id="product-template">
        <div
            ...
            <div
                ...
                <div class="text-size20 er-font-bold">Carbon 1 Pro</div>
...
onMounted(() => {
    ...
    function saveCurrentAnswer() {
        ...
        console.log(currentStep, $input.val())
    ...
    $('#start-quiz-btn').off('click').on('click', function () {
        userAnswers.fill(null);
        $('#email-form').find('button[type="submit"]').remove(); // Clean up button if added before
        $('input[type="radio"]').prop('checked', false);
        $('#height-select').val('');
